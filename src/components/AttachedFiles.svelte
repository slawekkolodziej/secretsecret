<script lang="ts">
  ...
  export let onDelete: ((file: FileData, i: number) => void) | undefined = undefined;
    ...
    {#each files as file, index}
        <div class="flex items-center justify-between bg-gray-50 p-2 rounded">
            <div class="flex flex-1 items-center space-x-2 overflow-hidden">
                ...
                <span class="text-sm text-gray-700 flex-1 overflow-hidden overflow-ellipsis text-nowrap">{file.name}</span>
                <span class="text-xs text-gray-500">({file.size < 1024 ? `${file.size} B` : file.size < 1024 * 1024 ? `${(file.size / 1024).toFixed(1)} KB` : `${(file.size / (1024 * 1024)).toFixed(1)} MB`})</span>
            </div>
            {#if onDelete}
                <button
                    type="button"
